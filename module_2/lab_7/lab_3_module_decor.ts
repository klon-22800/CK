...
export namespace Transport {
    ...
    export class VehicleImpl implements Vehicle {
        ...
        getVehicleVINOwnerReg(): string {
            return `VIN: ${this._vin}, Владелец: ${this._owner.lastName} ${this._owner.firstName} ${this._owner.patronymic}, Регистрационный номер: ${this._registrationNumber}`;
        }
    ...
    export class VehicleStorageImpl<T extends Vehicle> implements VehicleStorage<T> {
        ...
        sortByBrand(): T[] {
            const vehiclesCopy = [...this._vehicles];
            vehiclesCopy.sort((a, b) => a.brand.localeCompare(b.brand));
            return vehiclesCopy;
        }
